...
When the request contains a body (as in POST/PUT/PATCH methods) clients SHOULD include a SHA256 hash of the request in a `payload` tag as hex (`["payload", "<sha256-hex>"]`), servers MAY check this to validate that the requested payload is authorized.

Some applications overload the `payload` value to commit to different data, such as [NIP-96](96.md). Unless otherwise specified, `payload` should be calculated as the hash of the entire request body (`sha256(<http-request-body>)`).
