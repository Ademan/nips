...
When the request contains a body (as in POST/PUT/MATCH methods) specialized clients MAY include a SHA256 hash over an application specific set of data in an application specific tag instead of, or in addition to the `payload` tag. Servers implementing these specific applications SHOULD check this to validate the requested payload is authorized.

| Application     | Extra Tag Name |
| --------------- | -------------- |
| [NIP-96](96.md) | `payload_file` |
