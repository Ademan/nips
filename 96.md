...
When indicated, `clients` must add an [NIP-98](98.md) `Authorization` header (**optionally** with the encoded `payload` tag set to the hex-encoded 256-bit SHA-256 hash of the file - not the hash of the whole request body).
